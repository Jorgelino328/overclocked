...
func _ready():
	# Wait one frame for the player to initialize
...
func setup_hearts():
	...
	for i in range(total_hearts):
		...
		# IMPORTANT: Create a completely new AtlasTexture instance for every heart
...
func update_hearts(current_hp, max_hp):
	# Refresh layout if max_hp changed
	...
	for i in range(hearts.size()):
		...
		# Determine how much HP is in THIS specific heart (0, 5, or 10)
		var hp_in_this_heart = clamp(current_hp - (i * HP_PER_HEART), 0, 10)
		
		# Update the region based on the value
		if hp_in_this_heart >= 10:
			atlas.region = Rect2(0, 0, 16, 16)  # Full
		elif hp_in_this_heart >= 5:
